...
def home():
    if request.method == 'POST':
        ...
        # Check if the username exists in patient_info table
        connection = get_db_connection()
        cursor = connection.cursor()
        try:
            cursor.execute("SELECT COUNT(*) FROM patient_info WHERE username = %s", (username,))
            ...
            # Check if a prediction already exists for the username
            ...
            if result[0] > 0:
                ...
                cursor.execute(
                    "UPDATE predictions SET symptoms = %s, predicted_disease = %s WHERE username = %s",
                    (', '.join(user_symptoms), predicted_disease, username)
                )
            else:
                # Insert new prediction
                cursor.execute(
                    "INSERT INTO predictions (username, symptoms, predicted_disease) VALUES (%s, %s, %s)",
                    (username, ', '.join(user_symptoms), predicted_disease)
                )
            connection.commit()
